...
var TitleScreen = function TitleScreen(title,subtitle,callback) {
  ...
  function wrapText(ctx, text, width, y, lineHeight) {
    ...
    for (var n = 0; n < words.length; n++) {
        ...
        if (testWidth > width) {
            var lineMeasure = ctx.measureText(line);
            var x = (width - lineMeasure.width) / 2;
    ...
    var lineWidth = ctx.measureText(line);
    ctx.fillText(line, width/2 - lineWidth.width/2, y);
    return lineCount;
  }
...
var SurpriseScreen = function SurpriseScreen(subtitle,title,callback) {
  var up = false;
  this.step = function(dt) {
    if(!Game.keys['fire']) up = true;
    if(up && Game.keys['fire'] && callback) callback();
  ...
  this.draw = function(ctx) {
    ...
    // ctx.fillText(title,Game.width/2 - measure.width/2,300);
    if (true) {
